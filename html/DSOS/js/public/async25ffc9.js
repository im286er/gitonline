...
function e(i,e,o,m){
var n=i+"_"+o;
h.musicSupport&&u.tap(a("qqmusic_play_"+n),function(){
var i=h.musicList[n];
i.player.play({
mid_str:m
});
}),1*window.show_comment===1&&u.tap(a("qqmusic_love_icon_"+n),function(){
t(i,e,o),s(e);
});
}
function t(i,e,t){
var o=e+"_"+t,n=a("qqmusic_main_"+o),s="";
if(n){
p.hasClass(n,"loved")?(s="0","undefined"!=typeof h.loverCount[e]&&(h.loverCount[e]-=1)):(s="1",
"undefined"!=typeof h.loverCount[e]&&(h.loverCount[e]+=1));
for(var c=0;c<h.musicLen;c++){
var u=a("qqmusic_main_"+e+"_"+c);
u&&("0"===s?p.removeClass(u,"loved"):"1"===s&&p.addClass(u,"loved"));
}
m(s,e);
}
}
function o(){
"jump_detail"!=h.unloadStatus&&_.remove(h.cachekey);
var i=h.reportData;
i.musicid=i.musicid.join(";"),i.hasended=i.hasended.join(";"),i.commentid=i.commentid.join(";"),
i.mtitle=i.mtitle.join(";#"),i.detail_click=i.detail_click.join(";"),v.musicreport({
data:i
});
}
function m(i,e){
...
function n(){
var i=_.get(h.cachekey);
if(i){
try{
if(i=JSON.parse(i)||{},!i.time||(new Date).getTime()-h.cacheTime>1*i.time)return;
}catch(e){
return;
}
for(var t in h.musicList)if(i[t]){
var o=h.musicList[t],m=o.player,n=i[t].playTime;
1*n>0&&!i[t].paused&&(m.play({
...
function s(i){
for(var e=0;e<h.musicLen;e++){
var t=a("love_text_"+i+"_"+e);
t&&(t.innerText="undefined"!=typeof h.loverCount[i]&&1*h.loverCount[i]>0?1*h.loverCount[i]:"赞");
}
}
function c(){
var i=[];
for(var e in h.commentIdObj)i.push(e);
l.getLoveCount({
comment_id:i.join(","),
callback:function(i){
for(var e=i.topic_list,t=0,o=e.length;o>t;t++){
var m=e[t];
h.loverCount[m.topic_id]=m.like_count;
for(var n=0;n<h.musicLen;n++){
var c=a("qqmusic_main_"+m.topic_id+"_"+n);
c&&(1*m.like_status===1?p.addClass(c,"loved"):p.removeClass(c,"loved"));
}
s(m.topic_id);
}
}
...
var u=i("biz_common/dom/event.js"),d=i("biz_common/tmpl.js"),r=i("biz_wap/utils/qqmusic_player.js"),p=(i("biz_wap/utils/ajax.js"),
i("biz_common/dom/class.js")),_=i("biz_wap/utils/localstorage.js"),l=i("pages/love_comment.js"),v=i("pages/report.js"),f=i("pages/version4video.js"),h={
imgroot:"https://imgcache.qq.com/music/photo/mid_album_68",
love_url:"/mp/interestlike?#wechat_redirect",
cachekey:"qqmusicStatus",
cacheTime:6e5,
musicSupport:f.isSupportMusic(),
musicList:{},
commentIdObj:{},
loverCount:{},
musicLen:0,
...
if(window.reportMid=[],f.isShowMpMusic()){
var g=a("js_content");
if(g){
var q=g.getElementsByTagName("qqmusic")||[];
if(!(q.length<=0)){
h.musicLen=q.length;
for(var j=h.reportData,b=0,w=0,y=q.length;y>w;w++){
var C=q[w],k={};
if(k.musicid=C.getAttribute("musicid"),k.comment_id=C.getAttribute("commentid"),
k.musicid&&"undefined"!=k.musicid&&k.comment_id&&"undefined"!=k.comment_id){
window.reportMid.push(k.musicid),k.media_id=C.getAttribute("mid"),k.posIndex=b++,
k.musicImgPart=C.getAttribute("albumurl")||"",k.music_img=h.imgroot+k.musicImgPart,
k.audiourl=C.getAttribute("audiourl"),k.singer=C.getAttribute("singer"),k.music_name=C.getAttribute("music_name"),
j.musicid.push(k.musicid),j.commentid.push(k.comment_id),j.hasended.push(0),j.mtitle.push(k.music_name),
j.detail_click.push(0),h.musicSupport?(k.musicSupport=!0,k.player=r({
id:k.musicid,
media_id:k.media_id,
comment_id:k.comment_id,
posIndex:k.posIndex,
play:function(){
var i=a("qqmusic_main_"+this.comment_id+"_"+this.posIndex);
i&&(p.addClass(i,"qqmusic_playing"),h.reportData.hasended[this.posIndex]=1,v.report({
type:1,
...
pause:function(){
var i=a("qqmusic_main_"+this.comment_id+"_"+this.posIndex);
i&&(p.removeClass(i,"qqmusic_playing"),v.report({
type:1,
comment_id:this.comment_id,
action:5
}));
...
x.innerHTML=d.render("qqmusic_tpl",k),C.parentNode.appendChild(x.children[0]),h.commentIdObj[k.comment_id]=1,
h.musicList[k.musicid+"_"+k.posIndex]=k,e(k.musicid,k.comment_id,k.posIndex,k.media_id);
}
}
return u.on(window,"unload",o),h.musicSupport&&n(),1*window.show_comment===1&&setTimeout(function(){
c();
},0),h.musicList;
}
}
...
window.reportVid=[];
for(var d=0,s=r.length;s>d;++d){
i=r[d];
var c=i.getAttribute("data-src"),m=i.className||"",a=i.getAttribute("src")||c;
if(n.isShowMpVideo()&&a&&0==a.indexOf("http://v.qq.com/iframe/player.html")){
var p=a.match(/[\?&]vid\=([^&]*)/),u=p[1],f=document.getElementById("js_content").offsetWidth,l=Math.ceil(3*f/4);
window.reportVid.push(u),a=["/mp/videoplayer?video_h=",l,"&scene=1&source=4&vid=",u,"&mid=",appmsgid,"&idx=",itemidx||idx,"&__biz=",biz,"&uin=",uin,"&key=",key,"&pass_ticket=",pass_ticket,"&version=",version,"&devicetype=",window.devicetype||""].join(""),
setTimeout(function(e,t,i,n){
return function(){
n.removeAttribute("style"),n.setAttribute("width",e),n.setAttribute("height",t+37),
n.setAttribute("src",i);
};
}(f,l,a,i),0);
}else if(c&&(c.indexOf("newappmsgvote")>-1&&m.indexOf("js_editor_vote_card")>=0||0==c.indexOf("http://mp.weixin.qq.com/bizmall/appmsgcard")&&m.indexOf("card_iframe")>=0||c.indexOf("appmsgvote")>-1||c.indexOf("mp.weixin.qq.com/mp/getcdnvideourl")>-1)){
if(c=c.replace(/^http:/,location.protocol),m.indexOf("card_iframe")>=0)i.setAttribute("src",c.replace("#wechat_redirect",["&uin=",uin,"&key=",key,"&pass_ticket=",pass_ticket,"&scene=",source,"&msgid=",appmsgid,"&msgidx=",itemidx||idx,"&version=",version,"&devicetype=",window.devicetype||""].join("")));else{
var g=c.indexOf("#wechat_redirect")>-1,v=["&uin=",uin,"&key=",key,"&pass_ticket=",pass_ticket].join("");
m.indexOf("vote_iframe")>=0&&(v+=["&appmsgid=",mid,"&appmsgidx=",idx].join(""));
...
if(c&&c.indexOf("mp.weixin.qq.com/mp/getcdnvideourl")>-1&&f>0){
var y=f,w=3*y/4;
i.width=y,i.height=w,i.style.setProperty&&(i.style.setProperty("width",y+"px","important"),
i.style.setProperty("height",w+"px","important"));
}
}
...
var r=document.getElementById("js_content").offsetWidth,d=Math.ceil(3*r/4)+37;
...
},"getElementsByClassName"in document)for(var x,b=document.getElementsByClassName("video_iframe"),d=0;x=b.item(d++);)x.setAttribute("scrolling","no"),
