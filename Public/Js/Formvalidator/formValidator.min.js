...
//----------------------------------------------------------------------------------------------------
// [插件描述] jQuery formValidator表单验证插件，它是基于jQuery类库，实现了js脚本于页面的分离。对一个表
//            单对象，你只需要写一行代码就可以轻松实现20种以上的脚本控制。现支持一个表单元素累加很多种
//            校验方式,采用配置信息的思想，而不是把信息写在表单元素上，能比较完美的实现ajax请求。
//----------------------------------------------------------------------------------------------------
// [作者姓名] 王宁	
// [作者邮箱] ningpanyun@yeah.net
// [群主交流] 1114391317
// [更新日期] 2014-08-16
// [程序版本] ver1.0
//====================================================================================================
var fv_scriptSrc=document.getElementsByTagName('script')[document.getElementsByTagName('script').length - 1].src;
var scriptSrcArray=fv_scriptSrc.split('/'); var jsName=scriptSrcArray[scriptSrcArray.length-1]; var themedir=fv_scriptSrc.replace(jsName, '');
document.write('<script type="text/javascript" src="'+themedir+'formValidatorRegex.js"></script>');
...
$.formValidator = {
	...
	initConfig : function(controlOptions) {
		    ...
		    $("#"+settings.formID).submit(function() {
		        if(settings.ajaxForm != null){
					...
					if (settings.ajaxCountValid > 0) {
						settings.onAlert(settings.ajaxPrompt); return false;
					}
		...
		$.ajax({async:false, type:"GET", url:themedir + "formValidatorTheme.js", dataType:"script",
			error:function() { alert('当前皮肤加载出错，请确认皮肤【formValidatorTheme.js】文件是否存在')}
		}); 
	},
	...
	serializeData : function(data)
	{
		...
		$.each(parmArray, function(index, data){
			...
			if(li_pos > 0){
				...
				{
					case "#" : value = $("#"+value.substring(1)).val(); break;
					default  : break;
				}
				var parm = name+"="+value;
				parmStringNew = parmStringNew=="" ? parm : parmStringNew + '&' + parm;
	...
	ajaxValid : function(e) {
		...
		var ls_url = setting.url;
		var ls_data = $.formValidator.serializeData(setting.data);
		//获取要传递的参数
		...
		parm = setting.randNumberName ? setting.randNumberName+"="+((new Date().getTime())+Math.round(Math.random() * 10000)) : ""; 
...
var formValidator_setting =  {
	...
	tipCss 		   : { left:10, top:-4, height:20, width:280},
	triggerEvent   : "blur",
	forceValid     : false,
	tipID          : null,
	pwdTipID       : null,
