...
var cartObj = {
		...
		addProduct: function(goods_id,g_info){
			...
			this.cartProduct[goods_id] = {gid:g_info.gid,number:1,gname:g_info.gname,gprice:gprice}; 
		},
...
$(document).ready(function(){
    $('.close-btn').click(function(){
        $('.box1').fadeOut(function(){ $('#screen').hide(); });
        return false;
    });
...
$(function(){
	$('#orderNav').on('click','li',function(){
		...
		//$('.order-content').eq(_thisIndex).css('display','block').addClass('fadeL').siblings().css('display','none');
		var cid = $(this).attr('cid');
		getProductContent(cid,'');
	});
	$('.media-left').on('click','img',function(){
		$('#mymodal').show();
	});
