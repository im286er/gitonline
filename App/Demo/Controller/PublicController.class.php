...
class PublicController extends Controller {
    ...
    public function login() {
		...
		if( !isset($_SESSION[C('USER_AUTH_KEY')]) || !\Common\Org\Cookie::get('mid') ) {
			...
			$this->redirect(U('Index/index@ce', '', false, true));
	...
	public function autoLogin(){
		...
		$member = M("member")->alias('m')->where(array("m.mname"=>I("get.username"), "m.mtype"=>2))
    ...
    public function logout(){
    	...
    	if(isset($_SESSION[C('USER_AUTH_KEY')]) && \Common\Org\Cookie::get('mid')) {
			...
			$this->success("退出成功", U('Public/login@ce', '', true, true));

		} else {

			$this->success("已经退出", U('Public/login@ce', '', true, true)); 	
