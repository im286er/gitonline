...
class MessageController extends MerchantController {
	...
	public function pushmsg() {
		if( IS_POST ) {
			...
			$data['putype'] = 3;
			$data['pmid'] = $this->mid;
			$data['pucid'] = serialize(array('type'=>2, 'list'=>'tag_'.$this->jid));

			if( $pid=M('pushContent')->add($data) ) {
				if( intval($_POST['ptype']) == 0 ) {//消息
					$info['title'] = $data['ptitle'];
					$info['time'] = date('Y-m-d H:i:s');
				//	preg_match("/src\s*=\s*([\"|']?)([^ \"'>]+\.(gif|jpg|jpeg|bmp|png))\\1/i", $data['pcontent'], $match);
					//$info['imageUrl'] = $match[2] ? "http://yd.dishuos.com/".ltrim($match[2], '/') : '';
					$info['content'] = strip_tags($data['pcontent']);
					$info['pid'] = $pid;
					$data['pcontent'] = JSON($info);
					$this->_IGtPushMessageToCidTrans( $data );
				} else {//通知
					$data['pcontent'] = strip_tags($data['pcontent']);
					$this->_IGtPushMessageToCidStart( $data );
				}
	...
	public function notice(){
	  ...
	  $page = new \Demo\Org\Page(M('Notice')->where($where)->count(), 9);
	...
	public function listmsg() {
		...
		$page = new \Demo\Org\Page(M('pushContent')->where(array('pmid'=>array('in', $midlist)))->count(), 6);
	...
	//特定用户－启动应用
	private function _IGtPushMessageToCidStart( $data=array() ) {
		$args = array(
			'title'					=> $data['ptitle'],
			'text'					=> $data['pcontent'],
			'isRing'				=> true,
		...
		$pucid = unserialize($data['pucid']);
		
		$app = M('merchantApp')->where('jid='.$this->jid)->field()->find();
		if( $pucid['type']==1 ) {
			$res = \Common\Org\IGPushMsg::getIGPushMsg(true, $app)->pushMessageToApp(1, json_encode($args), json_encode($mesg), array(), array(), array('ANDROID'));
		} else {
			$res = \Common\Org\IGPushMsg::getIGPushMsg(true, $app)->pushMessageToApp(1, json_encode($args), json_encode($mesg), array(), array(), array('ANDROID'));
		}
	}
	...
	private function _IGtPushMessageToCidTrans( $data=array() ) {
		...
		if( $pucid['type']==1 ) {
			$res = \Common\Org\IGPushMsg::getIGPushMsg(true, $app)->pushMessageToApp(4, json_encode($args), json_encode($mesg), array(), array(), array('ANDROID', 'IOS'));
		} else {
			$res = \Common\Org\IGPushMsg::getIGPushMsg(true, $app)->pushMessageToApp(4, json_encode($args), json_encode($mesg), array(), array(), array('ANDROID', 'IOS'));
		}
	}
