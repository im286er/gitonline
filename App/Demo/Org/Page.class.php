...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'%downPage%  <span>%nowPage%/%totalPage%</span> %upPage%');
    // 默认分页变量名
    protected $varPage;
	//URL规则
	protected $urlrules;
    ...
    public function show() {
        ...
        if ($upRow>0) {
            $upPage     =   "<a href='".str_replace('__PAGE__', $upRow, $url)."' class='nextpage'></a>";
        } else {
            $upPage     =   "<a href='javascript:void(0);' class='nextpage'></a>";
        }

        if ($downRow <= $this->totalPages){
            $downPage   =   "<a href='".str_replace('__PAGE__', $downRow, $url)."' class='prepage'></a>";
        } else {
            $downPage   =   "<a href='javascript:void(0);' class='prepage'></a>";
        }
        ...
        $pageStr     =   str_replace(
            array('%nowPage%','%totalRow%','%totalPage%','%upPage%','%downPage%','%first%','%prePage%','%nextPage%','%end%'),
            array($this->nowPage,$this->totalRows,$this->totalPages,$upPage,$downPage,$theFirst,$prePage,$nextPage,$theEnd),$this->config['theme']);
        return $pageStr;
    }
