...
class PluginController extends MerchantController {
	...
	public function shareset(){
		...
		if( IS_POST ) {
			$types = I('post.types');
			if(array_key_exists($types,$sharetype)==false){
		...
		if($types=='download' && !$ShareData[$types]){
			...
			$ShareData[$types]['text'] = "【好店大爆料】Duang ~我正在".$mabbreviation."，体验".$active."，现在登录/下载还有红包、抵价券哦！";
		}else{
		//	if(!$ShareData[$types]['text'])$ShareData[$types]['text'] = '我正在使用帝鼠OS提供的物联网营销解决方案，硬件帮我拉客、软件帮我续客，微信跟支付宝都在帮我倒流。';
		}

		$this->assign('types', $types);
		$this->assign('data', $ShareData[$types]);
	...
	public function appdownset(){
		...
		$this->assign('modulelink', 'http://yd.dishuos.com/App/index/jid/'.$this->jid.'.html');
	
		
		$extend = M('merchant_extend')->find($this->jid);
		$app = D('App');
		if($extend['app_down']== 0 || !$extend){
			$result = $app->merchant_sync($this->jid);//开通同步
			if($result)$app->open_app_sync($this->jid);
		}
		if(IS_POST){
