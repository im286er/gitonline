...
/* * *用户中心 * * */
class MyController extends RebateviewController {
	...
	public function favorite(){
		...
		$sql = "SELECT f.*,max(case when g.gdprice>0 then g.gdprice*g.gvrebate else g.goprice*g.gvrebate end) as gfl FROM azd_goods as g,azd_fl_collection as f where f.flc_shopid=g.sid and f.flc_userid = ".$this->userid." and g.gstatus=1 GROUP BY f.flc_shopid order by f.flc_addtime desc ";
		...
		foreach($shops as $k2=>$v2){
			...
			$shops[$k2]['v_fl'] = D('Shop')->calculateRebate($value['gfl']);
			
			$shops[$k2]['p_fl'] = D('Shop')->calculateRebate($value['gfl'],'general');
		}
	...
	public function makePQrcode(){
		$pz_id = I('pz_id');
		$size = 10;
		$qcUrl = U('Order/prizeScan@app',array('pz_id'=>$pz_id));
		vendor("phpqrcode.phpqrcode");
		$QRcode = new \QRcode();
		echo $QRcode::png($qcUrl, false, 'H', $size);
	}
