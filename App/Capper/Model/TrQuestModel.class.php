...
class TrQuestModel extends Model {
	...
	public function inviteQuest($userid,$questid){
		...
		/***更新账户金额和收入增加日志***/
		$translation = array();
		$translation['t_uid'] = $userid;
		$translation['t_oid'] = 'quest'.str_pad($result,'11',"0",STR_PAD_LEFT);
		$translation['t_balance'] = $configQuest['money'];
		$translation['t_addtime'] = date('Y-m-d H:i:s');
		$translation['t_type'] = 5;
		$translation['t_notes'] = '领取邀请任务奖励';
		$update = D('TrUser')->where(array('u_userid'=>$userid))->setInc('u_balance',$configQuest['money']);
		if($update){//账户增加之后处理
			D('TrTranslation')->add($translation);
	...
	public function consumptionQuest($userid,$questid,$corder=0){
		...
		if(!$verifyAward['t_tid']){
			$verifyAward = $this->getOrder($userid,$configQuest['questname']);
		}
		if(!$verifyAward[0]['o_backprice'])return array('msg'=>'任务未完成','status'=>0);
		$money = round($verifyAward[0]['o_backprice']*$configQuest['rate'],2);
		$array = array();
		$array['uid'] = $userid;
		$array['questname'] = $configQuest['questname'];
		$array['finishtime'] = time();
		$array['awardmoney'] = $money;
		$array['assist'] = $verifyAward[0]['t_tid'];
		$result = $this->add($array);
		/***更新账户金额和收入增加日志***/
		$translation = array();
		$translation['t_uid'] = $userid;
		$translation['t_oid'] = 'quest'.str_pad($result,'11',"0",STR_PAD_LEFT);
		$translation['t_balance'] = $money;
		$translation['t_addtime'] = date('Y-m-d H:i:s');
		$translation['t_type'] = 5;
		$translation['t_notes'] = '领取订单返利任务奖励';
		$update = D('TrUser')->where(array('u_userid'=>$userid))->setInc('u_balance',$money);
		if($update){//账户增加之后处理
			D('TrTranslation')->add($translation);
			$this->where(array('id'=>$result))->setField('status','1');
		}
	...
	public function getOrder($userid,$questname){
		...
		$translation = M('TrTranslation')->alias('trt')->join('azd_tr_order tro on trt.t_oid=tro.o_id')->where(array('trt.t_type'=>'0','trt.t_uid'=>$userid))->order('trt.t_tid asc')->field('trt.t_tid,tro.o_gtype,tro.o_backprice')->limit($limit)->select();
		return $translation && $translation[0]['o_gtype']?$translation:false;
	}
	...
	public function verifyAward($userid){
		$questinfo = $this->where(array('questname'=>'consumption3','uid'=>$userid))->order('id desc')->find();
		if(!$questinfo)return true;
		$translation = M('TrTranslation')->alias('trt')->join('azd_tr_order tro on trt.t_oid=tro.o_id')->where(array('trt.t_type'=>'0','trt.t_uid'=>$userid,'trt.t_tid'=>array('egt',$questinfo['assist'])))->order('trt.t_tid desc')->field('trt.t_tid,trt.o_gtype,tro.o_backprice')->select();
		return $translation && $translation[0]['o_gtype']?$translation:false;
	}
