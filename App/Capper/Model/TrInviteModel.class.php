...
class TrInviteModel extends Model {
	...
	public function confirmRelation($userid,$marking=null){
		...
		if($invite){ //如果找到唯一编码
			...
			$result = D('TrUser')->where(array('u_userid'=>$userid))->setField('u_puserid',$invite['inviter']);//关联邀请会员
	...
	public function confirmBelong($userid,$jid){
		...
		$data = array();
		$data['u_sjid'] = $jid;
		if($magent)$data['u_sagentid'] = $magent;
		$result = D('TrUser')->where(array('u_userid'=>$userid))->setField($data);
		return $result;
	}
	...
	public function confirmCodeInvite($userid,$invitecode=null){
		...
		$inviteuser = D('TrUser')->find($inviter);
		...
		$data['inviteeuid'] = $userid;
		$this->data($data)->add();
		$result = D('TrUser')->where(array('u_userid'=>$userid))->setField('u_puserid',$inviter);//关联邀请会员
		return true;
	}
