...
/*首页* * * */
class AppController extends CapperappController {
	...
	public function guesslike(){
		...
		if($shopdata)foreach($shopdata as $key => $value){
			...
			$value['exterior'] = $value['exterior']?U('@yd').$value['exterior']:'';
			$value['linkurl'] = U('Shop/shopInfo@flapp',array('sid'=>$value['sid'],'superiorsource'=>'home','coordinate'=>$coordinate));
			$value['vipfl'] = '最高返现'.format_money(round($value['gfl']/100,2)).'元';
			$value['generalfl'] = '最高返现'.format_money(round(round($value['gfl']/100,2)*C('USER_RATION_VIP'),2)).'元';
			$shoplist[] =$value;
		}
		die(JSON($shoplist));
	}
	...
	public function shopMapLists() {
		...
		if($datalist)foreach($datalist as $key => $value){ 
				...
				$datalist[$key]['vipfl'] = '最高返现'.format_money(round($value['gfl']/100,2)).'元';
				$datalist[$key]['generalfl'] = '最高返现'.format_money(round(round($value['gfl']/100,2)*C('USER_RATION_VIP'),2)).'元';
