...
class Mail{
	...
	private $commandFunc = null; //当前邮件命令发送方式
	private $state       = true; //邮件命令返回状态
	public  $error       = null; //错误记录
	public  $debug       = true;//开启调试
	public  $mailformat  = 1;    //邮件格式 0=普通文本 1=html邮件
	
	public function __construct($config_arr, $debug = false){
		
		$host = gethostbyname($config_arr['mail_host']);
		$this->host  = $host;
		$this->port  = $config_arr['mail_port'];
		$this->from  = $config_arr['mail_user'];
		$this->user  = base64_encode($config_arr['mail_user']);
		$this->pass  = base64_encode($config_arr['mail_pwd']);
		$this->debug = $config_arr['mail_debug'];
		if(function_exists('socket_create')){
			$this->func = 'socket_create';
			$this->commandFunc = '_socketCommand';
			$this->showDebug('检测函数 '.$this->func.' 通过');
		}elseif(function_exists('fsockopen')){
			$this->func = 'fsockopen';
			$this->commandFunc = '_fsocketopenCommand';
			$this->showDebug('检测函数 '.$this->func.' 通过');
		}else {
	...
	private function _doSend(){
		...
		$All= "From:".$this->from."\r\n";
		$All.= "To:".$this->to."\r\n";
		$All.= "Subject:=?utf-8?B?".base64_encode($this->subject)."?=\r\n";
		...
		//以下是和服务器会话
		$this->{$this->commandFunc}("EHLO HELO\r\n");
		...
		$this->{$this->commandFunc}("AUTH LOGIN\r\n");
		if(!$this->state) return false;
		
		$this->{$this->commandFunc}($this->user."\r\n");
		if(!$this->state) return false;
		
		$this->{$this->commandFunc}($this->pass."\r\n", '235', 'smtp 认证失败');
		if(!$this->state) return false;

		$this->{$this->commandFunc}("MAIL FROM:<".$this->from.">\r\n", '250', '邮件发送失败');
		if(!$this->state) return false;
		
		$this->{$this->commandFunc}("RCPT TO:<".$this->to.">\r\n", '250', '邮件发送失败');
		if(!$this->state) return false;
		
		$this->{$this->commandFunc}("DATA\r\n", '354', '邮件发送失败');
		if(!$this->state) return false;
		
		$this->{$this->commandFunc}($All."\r\n.\r\n", '250', '邮件发送失败');
		if(!$this->state) return false;
		
		$this->{$this->commandFunc}("QUIT\r\n");
		if(!$this->state) return false;
	...
	//socket_create函数发送命令
	private function _socketCommand($send, $checkCode = '', $error=''){
	...
	//fsockopen函数命令发送
	private function _fsocketopenCommand($send, $checkCode = '', $error=''){
