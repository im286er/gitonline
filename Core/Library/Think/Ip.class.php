...
class Ip
{
	...
	function getlocation($ip = '')
	{
		...
		if(strstr($location['country'],"省")){
			$arr=explode("省",$location['country']);
			$location['province']=$arr[0]."省";
			$location['city']=$arr[1];
		}elseif(strstr($location['country'],"市")){
			$location['province']=$location['country'];
			$location['city']=$location['country'];
		}
		//mb_convert_encoding($location['country'], "UTF-8//IGNORE", "GBK");
	...
	{
		if(!$filename) $filename =dirname(__FILE__).'/Ip/UTFWry.dat';
		if(!file_exists($filename)) exit('UTFWry.dat is not exists!');
