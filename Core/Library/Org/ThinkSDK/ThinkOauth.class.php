...
abstract class ThinkOauth{
	...
	public function __construct($token = null){
		...
		$this->Type = $this->rechristen(strtoupper(substr($class, 0, strlen($class)-3)));
		//获取应用配置
	...
	public function rechristen($str){
    ...
    public static function getInstance($type, $token = null) {
		...
		import("Org.ThinkSDK.sdk.{$name}");
		$sdk = trim("\ORG\ThinkSDK\ ").$name;
		return new $sdk($token);
    	if (class_exists("{$sdk}")) {
    		return new \ORG\ThinkSDK\QqSDK($token);
    	} else {
	...
	private function config(){
		//echo U('User/callback@yd',array('jid'=>I('get.jid'),'type'=>$config['CALLBACK']));exit;
		$config = C("THINK_SDK_{$this->Type}");
		if(!empty($config['AUTHORIZE']))
			$this->Authorize = $config['AUTHORIZE'];
		if(!empty($config['CALLBACK']))
			//$this->Callback = U('User/callback@yd',array('jid'=>I('get.jid'),'type'=>$config['CALLBACK']));
			$this->Callback = "http://yd.dishuos.com/User/callback?jid=".I('get.jid')."&type=".$config['CALLBACK'];
		else
	...
	public function getRequestCodeURL(){
		...
		if($this->Authorize){
			...
			if(is_array($_param)){
				...
				throw new \Think\Exception('AUTHORIZE配置不正确！');
			}
		}
