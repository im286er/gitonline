...
class WxPayDataBase
{
	...
	**/
	...
	/****App部分-----张世川改写****/
	public function UrlEnParams($values=array())
	{
		ksort($values);
		$buff = "";
	...
	public function appSignatrue($values=array()){
			ksort($values);
			$buff = "";
			foreach ($values as $k => $v){
				if($v != "" && !is_array($v)){
...
{	
	...
	{
		$this->values['input_charset'] = $value;
	}
	public function SetPartenr($value)
	{
		$this->values['partenr'] = $value;
	}
	/****App部分****/
