...
require_once dirname(__FILE__)."/../".C('WXCONFIGPATH')."/WxPayConfig.php";
...
class WxPayApi
{
	...
	/**
	 * 
	 * APP下单张世川，appOrder中out_trade_no、body、total_fee、trade_type必填
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function appOrder($inputObj, $access_token = null){
		...
		$package = $inputObj->UrlEnParams($values);

		$jsonData = array();
		$jsonData['appid'] = WxPayConfig::APPID;
		$jsonData['noncestr'] = self::getNonceStr();
		$jsonData['package'] = $package;
		$jsonData['timestamp'] = time();
		$jsonData['app_signatrue'] = sha1($inputObj->appSignatrue($jsonData));
		$jsonData['sign_getmod'] = 'sha1';
		print_r($jsonData);
		$response = self::http_post($url.'?access_token='.$access_token,$jsonData);
		return $response;
	}
