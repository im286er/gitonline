...
class Wechat
{
	...
	const JSTICKET_TOKEN_URL = '/ticket/getticket?';
	...
	*/
	...
	public function checkAuth($appid='',$appsecret=''){
		...
		$access_token_cache_name = 'wechat_access_token_'.$this->appid;
		$data = S($access_token_cache_name);
		...
		if ($result)
		{
			...
			S($access_token_cache_name,array('access_token'=>$this->access_token,'expires_in'=>time()+$expire),$expire);
			return $this->access_token;
		}
		return false;
	}
	...
	public function getJsApiTicket(){
		...
		$jsapi_ticket_cache_name = 'wechat_jsapi_ticket_'.$this->appid;
		$data = S($jsapi_ticket_cache_name);
		if($data && $data['expires_in'] > time()){
			$this->jsapi_ticket = $data['ticket'];
			return $this->jsapi_ticket;
		}
		//echo self::API_URL_PREFIX.self::JSTICKET_TOKEN_URL.'type=jsapi&secret='.$this->access_token;
		$result = $this->http_get(self::API_URL_PREFIX.self::JSTICKET_TOKEN_URL.'type=jsapi&access_token='.$this->access_token);
		if ($result)
		{
			...
			$expire = $json['expires_in'] ? intval($json['expires_in'])-100 : 3600;
			S($jsapi_ticket_cache_name,array('ticket'=>$json['ticket'],'expires_in'=>time()+$expire),$expire);
			return $this->jsapi_ticket;
		}
		return false;
	}
